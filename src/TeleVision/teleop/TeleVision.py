...
import numpy as np
import asyncio
# WebRTC imports done lazily inside the webrtc code path to avoid
# requiring aiortc/av when using image streaming mode
...
class OpenTeleVision:
    def __init__(self, img_shape, shm_name, queue, toggle_streaming, stream_mode="image", cert_file="./cert.pem", key_file="./key.pem", ngrok=False):
        ...
        if stream_mode == "webrtc":
            # webrtc server -- lazy import to avoid requiring aiortc/av for image mode
            import ssl
            import logging
            import aiohttp_cors
